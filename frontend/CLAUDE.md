...
- `index.html` has `meta[name=theme-color]`, which `applyTheme` updates. `public/` holds the two logo SVGs, served from `/`: `hugginghack-mark.svg` (light theme and favicon) and `hugginghack-mark-dark.svg` (light brackets for dark backgrounds). Pick one with `brandMark(theme)` from `theme.ts`, never a hard-coded path.
