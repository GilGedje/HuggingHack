...
export function UploadChangeDialog({ model, directory, onClose, onQueued }: UploadChangeDialogProps) {
  ...
  return (
    <DialogFrame labelledBy="upload-change-title" className="upload-change-dialog" closing={closing} onDismiss={close} onSubmit={submit}>
      <header className="use-model-header">
        <div>
          <span className="eyebrow">Commit a change</span>
          <h2 id="upload-change-title">{model.id}</h2>
        </div>
        <button ref={closeButton} type="button" className="icon-button" onClick={close} aria-label="Close">
          <X size={20} />
        </button>
      </header>
